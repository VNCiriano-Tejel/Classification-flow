...
def add_new_features(df):
    """Adds the logarithm of some features"""
    df['log_feature_1'] = np.log(df['feature_1']-df["feature_1"].min()+1e-1)
    df['log_feature_2'] = np.log(df['feature_2']-df["feature_2"].min()+1e-1)
    df['log_feature_3'] = np.log(df['feature_3']-df["feature_3"].min()+1e-1)
    df['log_feature_4'] = np.log(np.abs((df['feature_4']-df["feature_4"].max()))+1) # feature 4 has the tail on the other side so we convert it to
    return df

def train_and_test_data(df):
    """selects the train and data set. They are always the same"""
    from sklearn.model_selection import train_test_split
    experiments= np.unique(df["exp_no"])
    exp_train_set, exp_test_set = train_test_split(experiments, test_size=0.2, random_state=19) # it always chooses the same test set

    
    train_set=df.loc[df['exp_no'].isin(exp_train_set), :]
    test_set=df.loc[df['exp_no'].isin(exp_test_set), :]
...
def feat_scaling(df):
    ...
    num_attribs= np.setxor1d(list(df.columns))
...
def prepare_for_LOF(filename):
    data=pd.read_csv(filename)

    # adds new features
    data=add_new_features(data)
    data=data.drop(["feature_1", "feature_2","feature_3","feature_4"], axis=1)

    # separate into train and test
    train_set, test_set=train_and_test_data(data) 

    # separate labels and train data
    train_set, train_labels=separate_labels(train_set)
    test_set, test_labels=separate_labels(test_set)

    # drops time and experiement number
    x_train=train_set.drop(["exp_no", "time", "depth"],axis=1)
    x_test=test_set.drop(["exp_no", "time", "depth"],axis=1)
    ...
    # Converts into panda again without time or experiement number
